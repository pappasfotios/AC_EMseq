...
### annotation file
bed <- read.table(file="newCpGi_plusShores.bed",header=F,
                  sep="\t",
                  stringsAsFactors = F)
...
cor_matrix_cpgi <- as.matrix(with(MEtraitCor, tapply(cor, list(trait, module), mean)))
p_matrix_cpgi <- as.matrix(with(MEtraitCor, tapply(p, list(trait, module), mean)))
