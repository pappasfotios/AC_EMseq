...
cor_matrix_prom <- dcast(MEtraitCor, trait ~ module, value.var = "cor")
rownames(cor_matrix_prom) <- cor_matrix_prom[,1]
cor_matrix_prom <- as.matrix(cor_matrix_prom[, -1])

p_matrix_prom <- dcast(MEtraitCor, trait ~ module, value.var = "p")
rownames(p_matrix_prom) <- p_matrix_prom[,1]
p_matrix_prom <- as.matrix(p_matrix_prom[, -1])
...
karyogram_prom <- ggbio::autoplot(combined_gr,layout = "karyogram", aes(color=module,fill=module)) +
  theme_void() +
