...
MEtraitCor <- getMEtraitCor(MEs, colData = colData[c(1:6,8:10)], corType = "pearson" ,file = "ME_Trait_Correlation_Stats_fi.txt")
traitDendro <- getCor(MEs, y = colData[c(1:6,8:10)], corType = "pearson", robustY = FALSE) %>% getDendro(transpose = TRUE)

cor_matrix_fi <- dcast(MEtraitCor, trait ~ module, value.var = "cor")
rownames(cor_matrix_fi) <- cor_matrix_fi[,1]
cor_matrix_fi <- as.matrix(cor_matrix_fi[, -1])

p_matrix_fi <- dcast(MEtraitCor, trait ~ module, value.var = "p")
rownames(p_matrix_fi) <- p_matrix_fi[,1]
p_matrix_fi <- as.matrix(p_matrix_fi[, -1])
...
karyogram_fi <- ggbio::autoplot(combined_gr,layout = "karyogram", aes(color=module,fill=module)) +
  theme_void() +
